...
brain_test_setup() {
  ...
  ROOT="$BRAIN_ROOT"; STATE="$ROOT/.state"; MIRROR="$ROOT/serlinolab"
  PERSONAL="$ROOT/personal/shared"; MARK="$ROOT/SOMETHING NEEDS YOUR ATTENTION.txt"
  LOCK="$STATE/run.lock"; LOG="$STATE/sync.log"; CONFLICT_STATE="$STATE/conflict_attempts"
  MAX_CONFLICT_ATTEMPTS=3
  export ROOT STATE MIRROR PERSONAL MARK LOCK LOG CONFLICT_STATE MAX_CONFLICT_ATTEMPTS
  mkdir -p "$STATE"
...
make_fake_personal_repo() { seed_repo "$BRAIN_ROOT/origin-personal.git" "$BRAIN_ROOT/personal/shared" note.txt; }
make_fake_mirror() { seed_repo "$BRAIN_ROOT/origin-mirror.git" "$BRAIN_ROOT/serlinolab" sub/file.txt; ONLINE_CHECK_REMOTE="$BRAIN_ROOT/origin-mirror.git"; export ONLINE_CHECK_REMOTE; run_sync_cycle; }
make_local_ahead_change() { echo 'unsent note' >> "$BRAIN_ROOT/personal/shared/note.txt"; }
